...
(function () {
  var HWT = window.HWT;
  var levels = HWT.levels || [];
  levels.sort(function (a, b) { return a.id - b.id; });

  var state = HWT.storage.load();
  ...
  function renderNav() {
    ...
    levels.forEach(function (lvl, i) {
      ...
      if (HWT.storage.levelCompleted(lvl.id, lvl.puzzles.length)) b.classList.add('completed');
    ...
    levels.forEach(function (lvl) {
      ...
      lvl.puzzles.forEach(function (_, idx) { if (HWT.storage.isSolved(lvl.id, idx)) done++; });
    });
  ...
  function goTo(idx) {
    ...
    current = idx;
    HWT.storage.setCurrentLevel(idx);
    renderLevel();
  ...
  function renderLevel() {
    ...
    if (typeof lvl.mountPlay === 'function') {
      try { lvl.mountPlay(playEl); }
        ...
        playEl.innerHTML = '<div class="muted">Could not load interactive piece. (' + HWT.escapeHtml(e.message) + ')</div>';
      }
    } else {
    ...
    HWT.glossaryRender(lvl.glossary || [], glossaryEl);
    notesEl.value = HWT.storage.getNote(lvl.id) || '';
  ...
  function renderPuzzle(lvl, puzzle, idx, container) {
    ...
    var solved = HWT.storage.isSolved(lvl.id, idx);
    ...
    HWT.hints.mount(actions, puzzle.hints || []);
    ...
    checkBtn.addEventListener('click', function () {
      ...
      if (result && result.correct) {
        ...
        if (!solved) {
          HWT.storage.markSolved(lvl.id, idx);
          solved = true;
  ...
  notesEl.addEventListener('input', function () {
    HWT.storage.setNote(levels[current].id, notesEl.value);
  });
