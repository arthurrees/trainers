...
(function () {
  var EHT = window.EHT;
  var levels = EHT.levels || [];
  levels.sort(function (a, b) { return a.id - b.id; });

  var state = EHT.storage.load();
  ...
  function renderNav() {
    ...
    levels.forEach(function (lvl, i) {
      ...
      if (EHT.storage.levelCompleted(lvl.id, lvl.puzzles.length)) b.classList.add('completed');
    ...
    levels.forEach(function (lvl) {
      ...
      lvl.puzzles.forEach(function (_, idx) { if (EHT.storage.isSolved(lvl.id, idx)) done++; });
    });
  ...
  function goTo(idx) {
    ...
    current = idx;
    EHT.storage.setCurrentLevel(idx);
    renderLevel();
  ...
  function renderLevel() {
    ...
    if (typeof lvl.mountPlay === 'function') {
      try { lvl.mountPlay(playEl); }
        ...
        playEl.innerHTML = '<div class="muted">Could not load interactive piece. (' + EHT.escapeHtml(e.message) + ')</div>';
      }
    } else {
    ...
    EHT.glossaryRender(lvl.glossary || [], glossaryEl);
    notesEl.value = EHT.storage.getNote(lvl.id) || '';
  ...
  function renderPuzzle(lvl, puzzle, idx, container) {
    ...
    var solved = EHT.storage.isSolved(lvl.id, idx);
    ...
    EHT.hints.mount(actions, puzzle.hints || []);
    ...
    checkBtn.addEventListener('click', function () {
      ...
      if (result && result.correct) {
        ...
        if (!solved) {
          EHT.storage.markSolved(lvl.id, idx);
          solved = true;
  ...
  notesEl.addEventListener('input', function () {
    EHT.storage.setNote(levels[current].id, notesEl.value);
  });
