...
(function () {
  var NT = window.NT;
  var levels = NT.levels || [];
  levels.sort(function (a, b) { return a.id - b.id; });

  var state = NT.storage.load();
  ...
  function renderNav() {
    ...
    levels.forEach(function (lvl, i) {
      ...
      if (NT.storage.levelCompleted(lvl.id, lvl.puzzles.length)) b.classList.add('completed');
    ...
    levels.forEach(function (lvl) {
      ...
      lvl.puzzles.forEach(function (_, idx) {
        if (NT.storage.isSolved(lvl.id, idx)) done++;
      });
    });
  ...
  function goTo(idx) {
    ...
    current = idx;
    NT.storage.setCurrentLevel(idx);
    renderLevel();
  ...
  function renderLevel() {
    ...
    if (typeof lvl.mountPlay === 'function') {
      try { lvl.mountPlay(playEl); }
        ...
        playEl.innerHTML = '<div class="muted">Could not load interactive piece. (' + NT.escapeHtml(e.message) + ')</div>';
      }
    } else {
    ...
    NT.glossaryRender(lvl.glossary || [], glossaryEl);

    notesEl.value = NT.storage.getNote(lvl.id) || '';
  ...
  function renderPuzzle(lvl, puzzle, idx, container) {
    ...
    var solved = NT.storage.isSolved(lvl.id, idx);
    ...
    NT.hints.mount(actions, puzzle.hints || []);
    ...
    checkBtn.addEventListener('click', function () {
      ...
      if (result && result.correct) {
        ...
        if (!solved) {
          NT.storage.markSolved(lvl.id, idx);
          solved = true;
  ...
  notesEl.addEventListener('input', function () {
    NT.storage.setNote(levels[current].id, notesEl.value);
  });
