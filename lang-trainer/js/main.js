...
(function () {
  var LT = window.LT;
  var levels = LT.levels || [];
  levels.sort(function (a, b) { return a.id - b.id; });

  var state = LT.storage.load();
  ...
  function renderNav() {
    ...
    levels.forEach(function (lvl, i) {
      ...
      if (LT.storage.levelCompleted(lvl.id, lvl.puzzles.length)) b.classList.add('completed');
    ...
    levels.forEach(function (lvl) {
      ...
      lvl.puzzles.forEach(function (_, idx) {
        if (LT.storage.isSolved(lvl.id, idx)) done++;
      });
    });
  ...
  function goTo(idx) {
    ...
    current = idx;
    LT.storage.setCurrentLevel(idx);
    renderLevel();
  ...
  function renderLevel() {
    ...
    if (typeof lvl.mountPlay === 'function') {
      try { lvl.mountPlay(playEl); }
        ...
        playEl.innerHTML = '<div class="muted">Could not load interactive piece. (' + LT.escapeHtml(e.message) + ')</div>';
      }
    } else {
    ...
    LT.glossaryRender(lvl.glossary || [], glossaryEl);

    notesEl.value = LT.storage.getNote(lvl.id) || '';
  ...
  function renderPuzzle(lvl, puzzle, idx, container) {
    ...
    var solved = LT.storage.isSolved(lvl.id, idx);
    ...
    LT.hints.mount(actions, puzzle.hints || []);
    ...
    checkBtn.addEventListener('click', function () {
      ...
      if (result && result.correct) {
        ...
        if (!solved) {
          LT.storage.markSolved(lvl.id, idx);
          solved = true;
  ...
  notesEl.addEventListener('input', function () {
    LT.storage.setNote(levels[current].id, notesEl.value);
  });
