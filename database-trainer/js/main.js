// main.js â€” boot, routing, level rendering
(function () {
  var DBT = window.DBT;
  var levels = DBT.levels || [];
  levels.sort(function (a, b) { return a.id - b.id; });

  var state = DBT.storage.load();
  ...
  function renderNav() {
    ...
    levels.forEach(function (lvl, i) {
      ...
      if (DBT.storage.levelCompleted(lvl.id, lvl.puzzles.length)) b.classList.add('completed');
    ...
    levels.forEach(function (lvl) {
      ...
      lvl.puzzles.forEach(function (_, idx) {
        if (DBT.storage.isSolved(lvl.id, idx)) done++;
      });
    });
  ...
  function goTo(idx) {
    ...
    current = idx;
    DBT.storage.setCurrentLevel(idx);
    renderLevel();
  ...
  function renderLevel() {
    ...
    if (typeof lvl.mountPlay === 'function') {
      try { lvl.mountPlay(playEl); }
        ...
        playEl.innerHTML = '<div class="muted">Could not load interactive piece. (' + DBT.escapeHtml(e.message) + ')</div>';
      }
    } else {
      playEl.innerHTML = '<div class="muted">No sandbox for this level â€” head to the puzzles below.</div>';
    }

    tryEl.innerHTML = '';
    lvl.puzzles.forEach(function (p, idx) { renderPuzzle(lvl, p, idx, tryEl); });

    DBT.glossaryRender(lvl.glossary || [], glossaryEl);

    notesEl.value = DBT.storage.getNote(lvl.id) || '';

    prevBtn.disabled = current === 0;
    nextBtn.disabled = current === levels.length - 1;
    nextBtn.textContent = (current === levels.length - 1) ? 'Done!' : 'Next â†’';
  ...
  function renderPuzzle(lvl, puzzle, idx, container) {
    ...
    var solved = DBT.storage.isSolved(lvl.id, idx);
    ...
    header.innerHTML =
      ...
      (solved ? 'âœ“ Solved' : 'Unsolved') + '</span>';
    ...
    DBT.hints.mount(actions, puzzle.hints || []);
    ...
    checkBtn.addEventListener('click', function () {
      ...
      if (result && result.correct) {
        ...
        feedback.innerHTML = 'âœ“ ' + (result.feedback || 'Correct!');
        if (!solved) {
          DBT.storage.markSolved(lvl.id, idx);
          solved = true;
          header.querySelector('.puzzle-status').textContent = 'âœ“ Solved';
          header.querySelector('.puzzle-status').classList.add('solved');
          renderNav();
        }
      } else {
        feedback.className = 'puzzle-feedback show incorrect';
        feedback.innerHTML = 'âœ— ' + ((result && result.feedback) || 'Not quite â€” try again.');
      }
    });
  ...
  notesEl.addEventListener('input', function () {
    DBT.storage.setNote(levels[current].id, notesEl.value);
  });
