...
(function () {
  ...
  function renderNav() {
    ...
    levels.forEach(function (lvl, i) {
      ...
      if (RXT.storage.levelCompleted(lvl.id, lvl.puzzles.length)) b.classList.add('completed');
    ...
    levels.forEach(function (lvl) {
      ...
      lvl.puzzles.forEach(function (_, idx) {
        if (RXT.storage.isSolved(lvl.id, idx)) done++;
      });
    });
  ...
  function goTo(idx) {
    ...
    current = idx;
    RXT.storage.setCurrentLevel(idx);
    renderLevel();
  ...
  function renderLevel() {
    ...
    if (typeof lvl.mountPlay === 'function') {
      try { lvl.mountPlay(playEl); }
        ...
        playEl.innerHTML = '<div class="muted">Could not load interactive piece. (' + RXT.escapeHtml(e.message) + ')</div>';
      }
    } else {
    ...
    // Glossary
    RXT.glossaryRender(lvl.glossary || [], glossaryEl);

    // Notes
    notesEl.value = RXT.storage.getNote(lvl.id) || '';
  ...
  function renderPuzzle(lvl, puzzle, idx, container) {
    ...
    var solved = RXT.storage.isSolved(lvl.id, idx);
    ...
    // Hint button after the check button
    RXT.hints.mount(actions, puzzle.hints || []);
    ...
    checkBtn.addEventListener('click', function () {
      ...
      if (result && result.correct) {
        ...
        if (!solved) {
          RXT.storage.markSolved(lvl.id, idx);
          solved = true;
  ...
  notesEl.addEventListener('input', function () {
    RXT.storage.setNote(levels[current].id, notesEl.value);
  });
