...
(function () {
  ...
  function renderNav() {
    ...
    levels.forEach(function (lvl, i) {
      ...
      if (GT.storage.levelCompleted(lvl.id, lvl.puzzles.length)) b.classList.add('completed');
    ...
    levels.forEach(function (lvl) {
      ...
      lvl.puzzles.forEach(function (_, idx) {
        if (GT.storage.isSolved(lvl.id, idx)) done++;
      });
    });
  ...
  function goTo(idx) {
    ...
    current = idx;
    GT.storage.setCurrentLevel(idx);
    renderLevel();
  ...
  function renderLevel() {
    ...
    if (typeof lvl.mountPlay === 'function') {
      try { lvl.mountPlay(playEl); }
        ...
        playEl.innerHTML = '<div class="muted">Could not load interactive piece. (' + GT.escapeHtml(e.message) + ')</div>';
      }
    } else {
    ...
    GT.glossaryRender(lvl.glossary || [], glossaryEl);

    notesEl.value = GT.storage.getNote(lvl.id) || '';
  ...
  function renderPuzzle(lvl, puzzle, idx, container) {
    ...
    var solved = GT.storage.isSolved(lvl.id, idx);
    ...
    GT.hints.mount(actions, puzzle.hints || []);
    ...
    checkBtn.addEventListener('click', function () {
      ...
      if (result && result.correct) {
        ...
        if (!solved) {
          GT.storage.markSolved(lvl.id, idx);
          solved = true;
  ...
  notesEl.addEventListener('input', function () {
    GT.storage.setNote(levels[current].id, notesEl.value);
  });
