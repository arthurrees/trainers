...
(function () {
  var AIT = window.AIT;
  var levels = AIT.levels || [];
  levels.sort(function (a, b) { return a.id - b.id; });

  var state = AIT.storage.load();
  ...
  function renderNav() {
    ...
    levels.forEach(function (lvl, i) {
      ...
      if (AIT.storage.levelCompleted(lvl.id, lvl.puzzles.length)) b.classList.add('completed');
    ...
    levels.forEach(function (lvl) {
      ...
      lvl.puzzles.forEach(function (_, idx) { if (AIT.storage.isSolved(lvl.id, idx)) done++; });
    });
  ...
  function goTo(idx) {
    ...
    current = idx;
    AIT.storage.setCurrentLevel(idx);
    renderLevel();
  ...
  function renderLevel() {
    ...
    if (typeof lvl.mountPlay === 'function') {
      try { lvl.mountPlay(playEl); }
        ...
        playEl.innerHTML = '<div class="muted">Could not load interactive piece. (' + AIT.escapeHtml(e.message) + ')</div>';
      }
    } else {
    ...
    AIT.glossaryRender(lvl.glossary || [], glossaryEl);
    notesEl.value = AIT.storage.getNote(lvl.id) || '';
  ...
  function renderPuzzle(lvl, puzzle, idx, container) {
    ...
    var solved = AIT.storage.isSolved(lvl.id, idx);
    ...
    AIT.hints.mount(actions, puzzle.hints || []);
    ...
    checkBtn.addEventListener('click', function () {
      ...
      if (result && result.correct) {
        ...
        if (!solved) {
          AIT.storage.markSolved(lvl.id, idx);
          solved = true;
  ...
  notesEl.addEventListener('input', function () {
    AIT.storage.setNote(levels[current].id, notesEl.value);
  });
